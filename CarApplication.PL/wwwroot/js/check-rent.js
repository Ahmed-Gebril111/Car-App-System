 ...
 checkAvailabilityForm.addEventListener('submit', function (e) {
   ...
   const isAvailable = (pickupDate > today );
   ...
   if (isAvailable) {
     ...
     resultDiv.innerHTML = "<p style='color:red; font-size: 25px; margin:30px;'>❌  Car is NOT available or datas not valied</p>";
 ...
 .then(function(response) {
   alert(`✅ Your booking has beeen successfully confirmed! \n A confirmation email will sent to ${email} containing the booking details `);
     // take care it's a ` (uppave tap) not '
 }, function(error) {
